...
import { GOOGLE_API_KEY, ACTION_CAL, EDUCATION_CAL, FUN_CAL,
          MEETING_CAL, RESOURCE_CAL, SHOWUP_CAL } from './config'
...
/* events that occur the same date of every month (i.e. the 1st, the 8th) */
...
/* events that occur the same day of the month (i.e. first Friday, last Monday) */
...
export default {
  ...
  getAllCalendars: (weeklyRecurrence, monthlyRecurrence) => Promise.map(calendars, (calendar) => {
    ...
    return axios.get(`https://content.googleapis.com/calendar/v3/calendars/${calendar.url}/events?key=${API_KEY}`)
      .then(res => {
        ...
        const monthly = reoccurring
          .filter(item => item.r[0] == "RRULE:FREQ=MONTHLY")
